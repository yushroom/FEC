...
{
	VSToPS vout;
	vout.Position = mul(float4(vin.Position.xyz, 1.0), MATRIX_MVP);
	vout.WorldPosition = mul(float4(vin.Position.xyz, 1.0), MATRIX_M).xyz;
	vout.TexCoord = vin.TexCoord;
	vout.WorldNormal = normalize(mul(float4(vin.Normal.xyz, 0.0), MATRIX_IT_M).xyz);
    vout.WorldTangent.xyz = normalize(mul(float4(vin.Tangent.xyz, 0.0), MATRIX_M).xyz);
...
#if HAS_specularGlossinessTexture
	FETexture2D(specularGlossinessTexture);
#endif
...
#if HAS_specularGlossinessTexture
	float4 specularGlossiness = texture2D(specularGlossinessTexture, v_UV);
	float3 specularColor = specularFactor * SRGBtoLINEAR(specularGlossiness.rgb);
	float glossiness = specularGlossiness.a;
#else
	float glossiness = glossinessFactor;
	float3 specularColor = specularFactor;
#endif
	float3 f0 = specularColor;
	float3 diffuseColor = baseColor.rgb * (1.0 - max(max(specularColor.r, specularColor.g), specularColor.b));
	
	alphaRoughness = (1 - glossiness);
